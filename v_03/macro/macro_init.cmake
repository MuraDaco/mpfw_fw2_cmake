...
macro(set_wsp_ver_macro MOD_NAME)
        ...
        foreach(X IN LISTS ${MOD_NAME}_DEPS_LIST)
            ...
            if(COL_LIST EQUAL 1)
 				if(${name}_WSP_VER)
 					if(NOT ${name}_WSP_VER STREQUAL "${X}")
                		message(FATAL_ERROR "DEBUG - set_wsp_ver_macro - MOD_NAME: ${MOD_NAME} - WSP version of ${name} library is different <${${name}_WSP_VER}> (old) vs <${X}> (new) :-(")
 					endif()
 				endif()
                set(${name}_WSP_VER    ${X})
            endif()
            if(COL_LIST EQUAL 2)
 				if(${name}_SRC_CFG)
 					if(NOT ${name}_SRC_CFG STREQUAL "${X}")
                		message(FATAL_ERROR "DEBUG - set_wsp_ver_macro - MOD_NAME: ${MOD_NAME} - SRC_CFG of ${name} library is different <${${name}_SRC_CFG}> (old) vs <${X}> (new) :-(")
 					endif()
 				endif()
                set(${name}_SRC_CFG    ${X})
            endif()
