...
macro(parsing_rpath_2 )

        set(TEST_LAYER 3)
        if(TEST_LAYER EQUAL 0)
            set(RPATH_TO_PARSE "trdp/sdk/stm/stm32f769_disco/layer/mpfw_fw2-trdp-sdk-stm-stm32f769_disco-0trdp/wsp/cmake/v_32stm0")
        elseif(TEST_LAYER EQUAL 1)
            set(RPATH_TO_PARSE "trdp/sdk/stm/stm32f769_disco/layer/mpfw_fw2-trdp-sdk-stm-stm32f769_disco-1trdp-usart/wsp/cmake/v_32stm0")
        elseif(TEST_LAYER EQUAL 2)
            set(RPATH_TO_PARSE "trdp/sdk/nxp/layer/mpfw_fw2-trdp-sdk-nxp-1trdp-usart/wsp/cmake/v_32stm0")
        elseif(TEST_LAYER EQUAL 3)
            set(RPATH_TO_PARSE "fnct/res/layer/mpfw_fw2-fnct-res-1trdp-sdk-stm-stm32f769_disco-usart/wsp/cmake/v_32stm0")
        elseif(TEST_LAYER EQUAL 4)
            set(RPATH_TO_PARSE "fnct/res/pippo/plutto/layer/mpfw_fw2-fnct-res-pippo-pluto-2wrpr-sdk-usart/wsp/cmake/v_32stm0")
        endif()
            ...
            ## parsing path to determine MODULE_TYPE, MODULE_FNCT, MODULE_OWNER & MODULE_RPATH
                ...
                set(MODULE_0NAME    ${MODULE_FNCT})
                set(MODULE_RPATH_OBLIGATORY "${MODULE_TYPE}/${MODULE_FNCT}" )
                if(MODULE_TYPE STREQUAL "trdp")
                    list(POP_FRONT OCCURENCES_LIST  MODULE_OWNER )
                    set(MODULE_0NAME    "${MODULE_0NAME}-${MODULE_OWNER}")
                    set(MODULE_RPATH_OBLIGATORY "${MODULE_RPATH_OBLIGATORY}/${MODULE_OWNER}" )
                endif()
                string(LENGTH "${MODULE_RPATH_OBLIGATORY}" MODULE_RPATH_OBLIGATORY_LENGTH)
                math(EXPR MODULE_RPATH_OBLIGATORY_LENGTH_PLUS_ONE "${MODULE_RPATH_OBLIGATORY_LENGTH} + 1")
                string(REPLACE "/" "-" MODULE_RPATH_OBLIGATORY_STR ${MODULE_RPATH_OBLIGATORY} )
                message("DEBUG - STEP 0 - MODULE_RPATH_OBLIGATORY:     ${MODULE_RPATH_OBLIGATORY}" )
                message("DEBUG - STEP 0 - MODULE_RPATH_OBLIGATORY_STR:     ${MODULE_RPATH_OBLIGATORY_STR}" )
                message("DEBUG - STEP 0 - MODULE_RPATH_OBLIGATORY_LENGTH:     ${MODULE_RPATH_OBLIGATORY_LENGTH}" )
                message("DEBUG - STEP 0 - RPATH_TO_PARSE:       ${RPATH_TO_PARSE}" )

                string(FIND "${RPATH_TO_PARSE}" "/layer/mpfw_fw2-${MODULE_RPATH_OBLIGATORY_STR}-" substring_pos)
                if(substring_pos GREATER_EQUAL MODULE_RPATH_OBLIGATORY_LENGTH)
                    string(SUBSTRING ${RPATH_TO_PARSE} 0 ${substring_pos} MODULE_RPATH)
                    string(REPLACE "/" "-" MODULE_RPATH_STR ${MODULE_RPATH} )
                else()
                    message(FATAL_ERROR "Module path is incorrect, it does not match the pattern \"${MODULE_RPATH_OBLIGATORY}/.../layer/mpfw_fw2-${MODULE_RPATH_OBLIGATORY_STR}-\" " )
                endif()

                message("DEBUG - STEP 0 - MODULE_RPATH:         ${MODULE_RPATH}" )
                set(MODULE_RPATH_N_PARTIAL_NAME "${MODULE_RPATH}/layer/mpfw_fw2-${MODULE_RPATH_OBLIGATORY_STR}" )
            ...
            ## determine MODULE_FNCT_ATTR
                ...
                math(EXPR substring_length "${substring_pos} - ${MODULE_RPATH_OBLIGATORY_LENGTH_PLUS_ONE}" )
                if(substring_length GREATER 0)
                    string(SUBSTRING ${MODULE_RPATH} ${MODULE_RPATH_OBLIGATORY_LENGTH_PLUS_ONE} ${substring_pos} MODULE_FNCT_ATTR)
            ...
            ## determine MODULE_0NAME 
                if(MODULE_FNCT_ATTR)
                    set(MODULE_0NAME    ${MODULE_0NAME}-${MODULE_FNCT_ATTR})
                endif()

            message("DEBUG - STEP 0 - MODULE_RPATH_STR:     ${MODULE_RPATH_STR}" )
            message("DEBUG - STEP 0 - MODULE_FNCT_ATTR:     ${MODULE_FNCT_ATTR}" )
            message("DEBUG - STEP 0 - MODULE_RPATH_OBLIGATORY_LENGTH_PLUS_ONE:     ${MODULE_RPATH_OBLIGATORY_LENGTH_PLUS_ONE}" )
            message("DEBUG - STEP 0 - MODULE_RPATH_N_PARTIAL_NAME:     ${MODULE_RPATH_N_PARTIAL_NAME}" )
            ...
            message(FATAL_ERROR "DEBUG - STEP 0")
                ...
                set(PARTIAL_RPATH   "/layer/mpfw_fw2-${MODULE_TYPE}-${MODULE_0NAME}-")

                string(FIND "${RPATH_TO_PARSE}" "${PARTIAL_RPATH}" substring_pos)
                if(substring_pos GREATER 0)
                    string(SUBSTRING ${RPATH_TO_PARSE} 0 ${substring_pos} MODULE_RPATH)
                else()
                    message(FATAL_ERROR "Module 0name is incorrect, it does not match the pattern \"${PARTIAL_RPATH}\" " )
                endif()

                string(LENGTH "${RPATH_TO_PARSE}"   string_length)
                string(LENGTH "${PARTIAL_RPATH}"    substring_length)

                math(EXPR substring_pos "${substring_pos} + ${substring_length}")
                math(EXPR substring_length "${string_length} - ${substring_pos}")

                string(SUBSTRING ${RPATH_TO_PARSE} ${substring_pos} ${substring_length} MODULE_LAYER)
                string(FIND "${MODULE_LAYER}" "/" substring_pos)
                string(SUBSTRING ${MODULE_LAYER} 0 ${substring_pos} MODULE_LAYER)

             message("DEBUG - STEP 0 - MODULE_LAYER:         ${MODULE_LAYER}" )
             message(FATAL_ERROR "DEBUG - STEP 0")

                string(REPLACE "-" ";" OCCURENCES_LIST ${MODULE_LAYER} )

                set(item_state STATE_0NAME)
                foreach(item IN LISTS OCCURENCES_LIST)
                    if(item_state STREQUAL "STATE_0NAME")
                        if()
                            set(item_state STATE_LAYER)
                        else()
                        endif()
                ...
                list(LENGTH OCCURENCES_LIST list_length)


                list(GET OCCURENCES_LIST 0 MODULE_0NAME )
                list(GET OCCURENCES_LIST 1 MODULE_LAYER )
                if(list_length GREATER 2)
                    list(GET OCCURENCES_LIST 2 MODULE_SUB_FNCT )
                    if(list_length GREATER 3)
                        list(GET OCCURENCES_LIST 3 MODULE_SUB_FNCT_2 )
                    endif()
                endif()
            ...
            message("DEBUG - STEP 0 - MODULE_0NAME:         ${MODULE_0NAME}" )
            message("DEBUG - STEP 0 - MODULE_LAYER:         ${MODULE_LAYER}" )
            message("DEBUG - STEP 0 - MODULE_SUB_FNCT:      ${MODULE_SUB_FNCT}" )
            message("DEBUG - STEP 0 - MODULE_SUB_FNCT_2:    ${MODULE_SUB_FNCT_2}" )
